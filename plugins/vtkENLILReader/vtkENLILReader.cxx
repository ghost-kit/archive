...
int vtkENLILReader::RequestData(vtkInformation* request,
  ...
  */
        ...
        for (k = 0; k < this->dimPhi; k++)
        {
            for (j = 0; j < this->dimTheta; j++)
            {
                for (i = 0; i < this->dimR; i++)
                {
                    ...
                    this->gridPoints->InsertNextPoint(xyz);
                     this->Radius->InsertNextValue(radiusValue);
                }
            }
        ...
        vtkDebugMacro(<< "NumberOfPoints after mesh read: " << this->gridPoints->GetNumberOfPoints());
    ...
    output->GetPointData()->AddArray(this->Radius);
    output->SetPoints(this->gridPoints);
  ...
  */
    ...
    for (j = 0; j < this->dimTheta; j++)
    {
        for (i = 0; i < this->dimR; i++)
        {
            cellScalar_Density->GetTuple(i + j*ni, tupleDbl);
            scalar = tupleDbl[0];
